...
import org.evertones.nasaOpenApi.v1.apiProperties.MarsRoverEarthDateParameter;
...
public class NasaOpenApiTest
    ...
    public void testGet10FirstPhotosByCuriosityOnEarthDateEqualsTo1000Sol() {
        MarsPhotos marsPhotos = given()
                .param(MarsRoverMartianSolParameter.API_KEY.getValue(), getNasaApiKey())
                .param(MarsRoverEarthDateParameter.EARTH_DATE.getValue(), EARTH_DATE_1000_SOL)
                .param(MarsRoverMartianSolParameter.PAGE.getValue(), "1")
                .when().log().all()
                .get(getMarsPhotosByCuriosityApiUrl().toString())
                .as(MarsPhotos.class);
        ...
        marsPhotos.getPhotos().stream().forEach(photo -> {
            assertEquals(photo.getEarthDate(), earthDate);
        });
    }
